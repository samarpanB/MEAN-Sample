    ...
    .directive("select2", ["$rootScope", "$timeout", "$parse", "$filter", "select2Config", "select2Stack", function ($rootScope, $timeout, $parse, $filter, select2Config, select2Stack) {
        ...
        return {
            ...
            link: function (scope, element, attrs, controller) {
                var getOptions;
                // @author samarpan
                // Edited :: to make sure jquery element is picked up when jqlite element is not available
                var select2el = angular.isUndefined(element.select2) ? $(element) : element;
                    ...
                    opts.query = function (query) {
                        ...
                        for (var i = 0; i < keys.length; i++) {
                            ...
                            if (label.toLowerCase().indexOf(query.term.toLowerCase()) > -1) {
                                // @author samarpan
                                // Edited :: To support opt groups
                                if(group) {
                                    if(gpMap.hasOwnProperty(group)) {
                                        gpMap[group].children.push({
                                            id: value,
                                            text: label,
                                            obj: values[key]
                                        });
                ...
                controller.$render = function () {
                    getSelection(function (selection) {
                        if (isMultiple) {
                            select2el.select2("data", selection);
                        } else {
                            select2el.select2("val", selection.id);
                        }
                    });
                };
                ...
                $timeout(function () {
                    ...
                    select2el.on("select2-blur", function () {
